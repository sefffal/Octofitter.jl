...
The G23H catalog is available for download. Due to its large size (~14 GB), you have two options:

**Option 1: Automatic download (first use)**
```julia
using Octofitter
# This will prompt you to download the catalog on first use
catalog_path = datadep"G23H_Catalog"
```

**Option 2: Use a local copy**
```julia
# Point directly to a local file
catalog_path = "/path/to/G23H-v1.0.feather"
```
...
For this example, we'll load a pre-existing catalog file:

```@example 1
# For documentation builds, we use a pre-loaded subset
# In practice, you would load the full catalog:
# catalog = DataFrame(Arrow.Table(datadep"G23H_Catalog"))

# For this example, we'll create the observation object directly
# using gaia_id lookup (requires catalog to be loaded)
nothing # hide
```

### Creating the Observation Object

The `GaiaHipparcosUEVAJointObs` object encapsulates all the G23H  data for a single star:

```julia
# Load the full catalog (do this once at the start of your session)
catalog = DataFrame(Arrow.Table("/path/to/G23H-v1.0.feather"))

# Create the observation object for your target
gaia_id = 756291174721509376  # Example: HD 91312
absastrom = GaiaHipparcosUEVAJointObs(;
    gaia_id=gaia_id,
    catalog=catalog,
    include_rv=true,  # Include Gaia RV variability constraints
)
```
...
chain, pt = octofit_pigeons(
    model,
    n_chains=32,
    n_rounds=8,  # Increase for production runs
