...
"""
Absolute radial velocity likelihood (for a planet).
"""
function Octofitter.ln_like(
    rvlike::StarAbsoluteRVLikelihood,
    θ_planet,
    ...
    # a tuple. Then we can index by inst_idxs to look up the right offset and jitter.
    barycentric_rv_inst = (
        hasproperty(θ_planet, :rv0_1) ? θ_planet.rv0_1 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_2) ? θ_planet.rv0_2 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_3) ? θ_planet.rv0_3 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_4) ? θ_planet.rv0_4 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_5) ? θ_planet.rv0_5 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_6) ? θ_planet.rv0_6 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_7) ? θ_planet.rv0_7 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_8) ? θ_planet.rv0_8 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_9) ? θ_planet.rv0_9 : convert(T, NaN),
        hasproperty(θ_planet, :rv0_10) ? θ_planet.rv0_10 : convert(T, NaN),
    )
    jitter_inst = (
    ...
    # Vector of radial velocity of the star at each epoch. Go through and sum up the influence of
    # each planet and put it into here. 
    rv_planet_buf_all_inst =  zeros(T, L) # @SArray
    noise_var_all_inst =   zeros(T, L) # @SArray

    M_planet = θ_planet.mass*Octofitter.mjup2msol
    M_tot = totalmass(planet_orbit)
    ...
    for inst_idx in 1:max_inst_idx
        ...
        # # Zygote version:
        # rv_planet_buf = @view(rv_planet_buf_all_inst[istart:iend]) .+ barycentric_rv_inst[inst_idx]
        ...
        if isnothing(rvlike.gaussian_process)
            ...
            try
                ...
                if err isa PosDefException
                    @warn "err" exception=(err, catch_backtrace()) 
                    return -Inf
                elseif err isa ArgumentError
                    @warn "err" exception=(err, catch_backtrace()) 
                    return -Inf
